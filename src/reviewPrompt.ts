 ...
 * a real number of distinct missing Info.plist usage-description keys flagged -- never on install, never on
...
const KEY_SEEN_FINDINGS = 'infoPlistUsageDescriptionCompanion.review.seenFindings';
const KEY_ANSWERED = 'infoPlistUsageDescriptionCompanion.review.answered';
...
function showPrompt(context: vscode.ExtensionContext): void {
  ...
  void vscode.window
    .showInformationMessage(
      `Info.plist Usage Description Companion: this has flagged ${HITS_BEFORE_PROMPT} real issues in your code -- if it's saved you time, a rating on the Marketplace helps other developers find it.`,
      rateAction,
      dismissAction,
    )
    .then((selection) => {
